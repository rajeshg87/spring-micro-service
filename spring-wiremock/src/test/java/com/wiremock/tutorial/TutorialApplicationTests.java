...
public class TutorialApplicationTests {
    ...
    @Test
    @Description("Test greet service")
    public void testGreet() {
    	//1. Stub the endpoint
        stubFor(get(urlPathMatching("/greet"))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody("Hi WireMock")
                )
        );
        
        String greetServer = "http://localhost:8070";
        
        //2. Make a request
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<String> response = restTemplate
                .getForEntity(fakeServer+"/greet", String.class);
        
        //3. Verify
        assertNotNull(response);
        assertTrue("Status code not equals to 200",response.getStatusCode().equals(HttpStatus.OK));
        assertTrue("Contains fail", response.getBody().contains("Hi WireMock"));
    }
